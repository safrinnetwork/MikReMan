...
function addPPPUser($input) {
    ...
    try {
        ...
        if (isset($input['createNatRule']) && !empty($input['remote_address'])) {
            ...
            if ($createMultipleNat && count($ports) > 1) {
                ...
                foreach ($ports as $port) {
                    ...
                    $nat_data = [
                        ...
                        'to-addresses' => $input['remote_address'],
...
function deletePPPUser($input) {
    ...
    try {
        ...
        // STEP 1: Delete NAT rules FIRST (before deleting user)
        $nat_deleted = 0;
        ...
        // STEP 2: Delete PPP user (after NAT rules are deleted)
        try {
