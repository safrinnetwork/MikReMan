                ...
                <div class="sidebar-header">
                    <div class="brand-container">
                        ...
                        <a href="admin.php" class="sidebar-brand">
                            <span class="brand-text">VPN Remote</span>
                            <small class="brand-subtitle">Management</small>
                        </a>
                    </div>
                </div>
        ...
        class PPPManager {
                    ...
                    <tr>
                        ...
                        <td>
                            <button class="btn btn-sm btn-outline-info btn-action" 
                                    onclick="pppManager.showUserDetails('${this.escapeHtml(user['.id'])}')"
                                    title="View Details">
                                <i class="bi bi-info-circle"></i>
                            </button>
                            <button class="btn btn-sm btn-outline-warning btn-action"
                                    onclick="pppManager.editUser('${this.escapeHtml(user['.id'])}')"
                                    title="Edit User">
                                <i class="bi bi-pencil"></i>
                            </button>
                            <button class="btn btn-sm ${user.disabled === 'true' ? 'btn-outline-success' : 'btn-outline-warning'} btn-action"
                                    onclick="pppManager.toggleUserStatus('${this.escapeHtml(user['.id'])}')"
                                    title="${user.disabled === 'true' ? 'Enable User' : 'Disable User'}">
                                <i class="bi bi-${user.disabled === 'true' ? 'unlock' : 'lock'}"></i>
                            </button>
                            <button class="btn btn-sm btn-outline-danger btn-action"
                                    onclick="pppManager.deleteUser('${this.escapeHtml(user['.id'])}')"
                                    title="Delete User">
            ...
            async bulkDeleteUsers() {
                ...
                try {
                    ...
                    const result = await this.fetchAPI('bulk_delete_ppp_users', { 
                        user_ids: Array.from(this.selectedUsers) 
                    }, 'POST');
                    
                    if (result.success) {
                        this.showAlert(`${this.selectedUsers.size} users deleted successfully!`);
            ...
            async bulkToggleStatus() {
                ...
                try {
                    ...
                    const result = await this.fetchAPI('bulk_toggle_ppp_users', { 
                        user_ids: Array.from(this.selectedUsers) 
                    }, 'POST');
                    
                    if (result.success) {
                        this.showAlert(`Status updated for ${this.selectedUsers.size} users!`);
                        this.selectedUsers.clear();
                        this.updateBulkActions();
                        await this.loadUsers();
                        await this.updateStats();
                    ...
                    this.showAlert('Error toggling user status: ' + error.message, 'danger');
                } finally {
                    this.hideLoading();
                }
            }
            ...
            updateSortHeaders() {
                ...
                if (currentHeader) {
                    currentHeader.className = this.sortDirection === 'asc' ? 
                        ...
                        'bi bi-sort-alpha-up text-primary';
                }
            }
        }
